...
1. Check that you have your .env.dev and .env.dev.db.  If for some reason these files are not generated, remember that you can find samples in the repository.
   ...
   SQL_DATABASE=roiback_dev
   SQL_USER=roiback
   SQL_PASSWORD=roiback
   SQL_HOST=db
   SQL_PORT=5432
   DATABASE=postgres
   ...
   POSTGRES_PASSWORD=roiback
   POSTGRES_DB=roiback_dev
    ```

2. Create docker volumes to save the data:
    ```sh
    $ docker volume create --name=roiback_postgres_data_dev
 ...
 * **Environment files:** Once the server is created, you must copy the .env files in the production server inside **/opt/roiback** path.
    ```sh
    $ cat .env.prod
    $ cat .env.prod.db
